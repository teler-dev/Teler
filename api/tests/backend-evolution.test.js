...
test('multi-user evidence and AI history respect employee and organization roles', () => {
  const employee = { id: 'employee-a', organization: { id: 'org-a', role: 'member' } };
  const manager = { id: 'manager-a', organization: { id: 'org-a', role: 'owner' } };
  ...
  assert.deepEqual(employeeQuery.values, ['screenshot-a', 'employee-a', 'org-a']);
  assert.match(managerQuery.text, /organization_id=\$2/);
  assert.deepEqual(managerQuery.values, ['screenshot-a', 'org-a']);

...
async function processEvidenceAiAnalysis(payload) {
  ...
  try {
    const candidates = await pool.query(`select s.id,s.storage_path,s.captured_at,s.active_app,s.active_window,s.visual_hash from app.screenshots s left join app.screenshot_ai_findings f on f.organization_id=s.organization_id and f.screenshot_id=s.id where s.organization_id=$1 and s.session_id=$2 and f.id is null ${payload.final ? '' : 'and s.captured_at < $3'} order by s.captured_at asc`, payload.final ? [context.organization_id, context.session_id] : [context.organization_id, context.session_id, windowEnd]);
    ...
    for (const shot of uniqueShots) {
      ...
      const result = await relay({ mode: 'screenshot', image_base64: image.toString('base64'), image_mime_type: evidenceMimeType(shot.storage_path), metadata: { captured_at: shot.captured_at, time_range: timeRange, active_app: shot.active_app, active_window: shot.active_window } });

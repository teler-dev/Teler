...
export const Dashboard:React.FC<Props>=({onLogout,userName='User',initialEmployee,onBack})=>{
  ...
  const deterministicSummary=selected?`Productivity ${selected.overall_productivity_score}/100, focus ${selected.focus_score}/100, ${idlePct}% idle time and ${switches} context switch${switches===1?'':'es'} across ${fmtDuration(selected.total_minutes)}.`:'';
  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 flex flex-col min-h-screen relative z-10">
      ...
      <main className="p-4 md:p-6 space-y-5 min-w-0">
        ...
        <Card as="section" padding="lg" className="flex flex-wrap gap-5 items-start justify-between"><div className="flex gap-4 min-w-0"><div className="w-14 h-14 rounded-2xl bg-accent-soft border border-accent text-accent font-bold text-xl flex items-center justify-center shrink-0">{employee.name.slice(0,1).toUpperCase()}</div><div className="min-w-0"><h2 className="text-xl font-bold truncate">{employee.name}</h2><p className="text-sm text-secondary mt-1">{employee.role||'Role not provided'}{employee.client?` · ${employee.client}`:''}</p><p className="text-xs text-secondary mt-2">{sessions.length} supporting session{sessions.length===1?'':'s'} in the current filter.</p></div></div>{selected&&<StatusBadge tone={scoreClass.label==='Elite'||scoreClass.label==='Strong'?'success':scoreClass.label==='Moderate'?'warning':'danger'}>{selected.overall_productivity_score}/100 · {scoreClass.label}</StatusBadge>}</Card>
        ...
        {loading&&!selected?<div className="space-y-4"><div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[0,1,2,3].map(i=><div key={i} className="h-28 bg-surface-card border border-subtle rounded-2xl skeleton-shimmer animate-shimmer"/>)}</div><div className="h-60 bg-surface-card border border-subtle rounded-2xl skeleton-shimmer animate-shimmer"/></div>:selected?<>
          ...
          ].map(card=><Card key={card.label} padding="md"><div className="flex items-center justify-between gap-2"><p className="text-sm text-secondary">{card.label}</p><span className="text-accent">{card.icon}</span></div><p className="text-2xl md:text-3xl font-bold mt-3">{card.value}</p><p className="text-xs text-secondary mt-1">{card.sub}</p></Card>)}</section>
          <section className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><div className="teler-ai-narrative bg-surface-card border border-subtle rounded-2xl p-5 md:p-6"><div className="flex items-center gap-2"><BrainCircuit className="w-4 h-4 text-accent"/><h3 className="font-semibold">AI session interpretation</h3><span className="ml-auto text-xs text-secondary">Confidence {confidence}%</span></div><p className="text-sm text-secondary leading-6 mt-4">{deterministicSummary}</p><div className="mt-5 pt-4 border-t border-subtle"><p className="text-sm font-semibold">Why this conclusion?</p><ul className="mt-2 space-y-2 text-sm text-secondary"><li>Session {selected.id} · {fmtDate(selected.created_at)}</li><li>Focus {selected.focus_score}/100 · idle {idlePct}% · {switches} context switches</li>{selected.main_tasks?.slice(0,2).map(task=><li key={task}>Observed task: {task}</li>)}</ul></div></div><div className="bg-surface-card border border-subtle rounded-2xl p-5 md:p-6"><div className="flex items-center gap-2"><BarChart3 className="w-4 h-4 text-accent"/><h3 className="font-semibold">Evidence</h3><span className="ml-auto text-xs text-secondary">{screenshots.length} screenshot{screenshots.length===1?'':'s'}</span></div>{screenshots.length?<div className="grid grid-cols-2 gap-2 mt-4">{screenshots.map((path,index)=><a key={path} href={screenshotUrl(path)} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-lg border border-subtle bg-surface-raised"><img src={screenshotUrl(path)} alt={`Screenshot ${index+1} from this session`} className="block w-full aspect-video object-cover"/><span className="block px-2 py-1.5 text-xs text-secondary">Screenshot {index+1}</span></a>)}</div>:evidenceApps.length?<div className="space-y-3 mt-4">{evidenceApps.slice(0,6).map(app=><div key={`${app.app}-${app.minutes}`}><div className="flex justify-between gap-3 text-sm"><span className="truncate">{app.app}</span><span className="text-secondary shrink-0">{fmtDuration(app.minutes)}</span></div><div className="h-1.5 bg-surface-raised rounded-full mt-1.5 overflow-hidden"><div className="h-full bg-accent rounded-full" style={{width:`${Math.min(100,app.minutes/Math.max(...evidenceApps.map(item=>item.minutes),1)*100)}%`}}/></div></div>)}</div>:<p className="text-sm text-secondary mt-4">No evidence was recorded for this session.</p>}</div></section>
          <section className="bg-surface-card border border-subtle rounded-2xl overflow-hidden"><div className="p-4 md:p-5 border-b border-subtle"><h3 className="font-semibold">Supporting sessions</h3><p className="text-sm text-secondary mt-1">Each session has its own shareable URL.</p></div>{sessions.map(session=>{const href=sessionPath(employee.name,session.id);return <a key={session.id} href={href} onClick={event=>{if(event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();setSelectedId(session.id);navigate(href)}}} className={`grid grid-cols-[1fr_auto] md:grid-cols-[1fr_auto_auto_auto] gap-3 items-center p-4 text-left border-b border-subtle hover:bg-surface-raised ${selected.id===session.id?'bg-accent/10':''}`}><span><span className="block text-sm font-semibold">{fmtDate(session.created_at)}</span><span className="block text-xs text-secondary mt-0.5 truncate">{session.main_tasks?.[0]||session.claimed_task||`Session ${session.id}`}</span></span><span className="text-sm font-semibold">{session.overall_productivity_score}</span><span className="hidden md:block text-sm text-secondary">{fmtDuration(session.total_minutes)}</span><span className="hidden md:block text-sm text-secondary">{session.app_switches?.length??0} switches</span></a>})}</section>

...
import { ArrowLeft, BarChart3, BrainCircuit, Clock, GitBranch, Keyboard, MousePointer2, RefreshCw, Zap } from 'lucide-react';
...
import { apiFetch, screenshotUrl } from '../../services/apiConfig';
...
import { filterSessionsByWorkspace, subscribeWorkspace } from '../../services/workspaceService';
...
export const Dashboard:React.FC<Props>=({onLogout,userName='User',initialEmployee,onBack})=>{
  ...
  const [autoReport,setAutoReport]=useState<{status:string;summary?:string;highlights?:string[];confidence?:number;error_message?:string;findings?:Array<{screenshot_id:string;summary:string;confidence?:number}>}|null>(null);
  useEffect(()=>subscribeWorkspace(()=>setFilterRevision(value=>value+1)),[]);
  const visibleSessions=useMemo(()=>filterSessionsByWorkspace(sessions),[sessions,filterRevision]);
  const alerts=useMemo(()=>generateAlerts(sessions),[sessions]);
  const selected=useMemo(()=>visibleSessions.find(session=>session.id===selectedId)??visibleSessions[0]??null,[visibleSessions,selectedId]);
  ...
  useEffect(()=>{let cancelled=false; const id=(selected?.id||'').replace(/^control:/,''); if(!/^[a-f0-9-]{36}$/i.test(id)){setAutoReport(null);return} apiFetch(`/api/v1/ai-reports/sessions/${id}`).then(async response=>response.ok?response.json():null).then(payload=>{if(!cancelled)setAutoReport(payload?.data??null)}).catch(()=>{if(!cancelled)setAutoReport(null)}); return()=>{cancelled=true}},[selected?.id]);
  ...
  const browserTabs=selected?.evidence?.browser_tabs??[];
  const keyPresses=Math.max(0,Number(selected?.key_count)||0),mouseClicks=Math.max(0,Number(selected?.mouse_clicks)||0);
    ...
    <div className="flex-1 ml-56 min-w-0 flex flex-col min-h-screen relative z-10">
      ...
      <PageContainer>
        ...
        {loading&&!selected?<LoadingState variant="detail" label="Loading employee telemetry" />:selected?<>
          <section className="bg-surface-card border border-subtle rounded-2xl p-3 flex flex-col sm:flex-row sm:items-center gap-2.5"><label className="text-xs font-semibold text-secondary shrink-0" htmlFor="session-select">Review session</label><Select id="session-select" className="w-full text-sm" value={selected.id} onChange={event=>{const id=event.target.value;setSelectedId(id);navigate(sessionPath(employee.name,id));}}>{visibleSessions.map(session=><option key={session.id} value={session.id}>{fmtDate(session.created_at)} · {fmtDuration(session.total_minutes)} · {session.overall_productivity_score>0?`${session.overall_productivity_score}/100`: 'awaiting telemetry'}</option>)}</Select><span className="text-xs text-muted shrink-0">{visibleSessions.length} shown</span></section>
          <KpiGrid>
          ...
          <section className="bg-surface-card border border-subtle rounded-2xl px-4 py-3.5 md:px-5" aria-label="Activity basis">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-semibold">Activity basis</h3><p className="text-xs text-secondary mt-0.5">The metrics above are based on recorded input and window changes.</p></div><div className="grid grid-cols-3 gap-2 sm:min-w-[390px]"><div className="rounded-lg bg-surface-raised px-3 py-2"><span className="flex items-center gap-1.5 text-[11px] text-secondary"><Keyboard className="w-3.5 h-3.5"/>Keyboard</span><strong className="block text-sm mt-0.5">{keyPresses}</strong></div><div className="rounded-lg bg-surface-raised px-3 py-2"><span className="flex items-center gap-1.5 text-[11px] text-secondary"><MousePointer2 className="w-3.5 h-3.5"/>Clicks</span><strong className="block text-sm mt-0.5">{mouseClicks}</strong></div><div className="rounded-lg bg-surface-raised px-3 py-2"><span className="flex items-center gap-1.5 text-[11px] text-secondary"><GitBranch className="w-3.5 h-3.5"/>Switches</span><strong className="block text-sm mt-0.5">{switches}</strong></div></div></div>
            <p className="text-xs text-secondary mt-3 pt-3 border-t border-subtle"><strong className="text-primary font-medium">This session:</strong> {fmtDuration(selected.active_minutes_estimate)} active from {keyPresses} keyboard inputs and {mouseClicks} mouse clicks; {fmtDuration(selected.idle_minutes_estimate)} idle after 30 seconds without input. Reading, mouse movement, and scrolling alone may still be counted as idle.</p>
          </section>
          <section className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><div className="teler-ai-narrative bg-surface-card border border-subtle rounded-2xl p-5 md:p-6"><div className="flex items-center gap-2"><BrainCircuit className="w-4 h-4 text-accent"/><h3 className="font-semibold">AI session interpretation</h3><span className="ml-auto text-xs text-secondary">{autoReport?autoReport.status.replace(/_/g,' '):`Confidence ${confidence}%`}</span></div><p className="text-sm text-secondary leading-6 mt-4">{autoReport?.summary||deterministicSummary}</p>{autoReport?.confidence!==undefined&&<p className="text-xs text-secondary mt-2">Evidence confidence {Math.round(autoReport.confidence*100)}%</p>}{autoReport?.highlights?.length?<div className="mt-4"><p className="text-sm font-semibold">Key evidence</p><ul className="mt-2 space-y-1.5 text-sm text-secondary">{autoReport.highlights.slice(0,5).map((highlight,index)=><li key={`${highlight}-${index}`}>• {highlight}</li>)}</ul></div>:null}<div className="mt-5 pt-4 border-t border-subtle"><p className="text-sm font-semibold">Why this conclusion?</p><ul className="mt-2 space-y-2 text-sm text-secondary"><li>Supporting session · {fmtDate(selected.created_at)}</li><li>Focus {selected.focus_score>0?`${selected.focus_score}/100`:'not scored'} · idle {idlePct}% · {switches} context switches</li>{autoReport?.findings?.slice(0,2).map(finding=><li key={finding.screenshot_id}>{finding.summary}</li>)||selected.main_tasks?.slice(0,2).map(task=><li key={task}>Observed task: {task}</li>)}</ul></div></div>{screenshots.length?<EvidenceGallery imageUrls={screenshots.map(screenshotUrl)} />:<div className="bg-surface-card border border-subtle rounded-2xl p-5 md:p-6"><div className="flex items-center gap-2"><BarChart3 className="w-4 h-4 text-accent"/><h3 className="font-semibold">Evidence</h3></div>{evidenceApps.length?<div className="space-y-3 mt-4">{evidenceApps.slice(0,6).map(app=><div key={`${app.app}-${app.minutes}`}><div className="flex justify-between gap-3 text-sm"><span className="truncate">{app.app}</span><span className="text-secondary shrink-0">{fmtDuration(app.minutes)}</span></div><div className="h-1.5 bg-surface-raised rounded-full mt-1.5 overflow-hidden"><div className="h-full bg-accent rounded-full" style={{width:`${Math.min(100,app.minutes/Math.max(...evidenceApps.map(item=>item.minutes),1)*100)}%`}}/></div></div>)}</div>:<p className="text-sm text-secondary mt-4">No evidence was recorded for this session.</p>}</div>}</section>
          {browserTabs.length?<section className="bg-surface-card border border-subtle rounded-2xl p-5 md:p-6"><h3 className="font-semibold">Browser tabs observed</h3><p className="text-sm text-secondary mt-1">Links are captured without search terms, fragments, or page content.</p><ul className="mt-4 grid gap-2 md:grid-cols-2">{browserTabs.map((tab,index)=><li key={`${tab.url}-${index}`} className="min-w-0"><a className="block rounded-lg border border-subtle px-3 py-2 text-sm hover:border-accent focus:outline-none focus:ring-2 focus:ring-accent" href={tab.url} target="_blank" rel="noreferrer"><span className="block truncate font-medium">{tab.title}</span><span className="block truncate text-xs text-secondary mt-0.5">{tab.url}</span></a></li>)}</ul></section>:null}
          <DataList><div className="p-4 md:p-5 border-b border-subtle"><h3 className="font-semibold">Supporting sessions</h3><p className="text-sm text-secondary mt-1">Each session has its own shareable URL.</p></div>{visibleSessions.map(session=>{const href=sessionPath(employee.name,session.id);return <DataListLink key={session.id} href={href} onClick={event=>{if(event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();setSelectedId(session.id);navigate(href)}}} className={`grid grid-cols-[1fr_auto] md:grid-cols-[1fr_auto_auto_auto] gap-3 items-center ${selected.id===session.id?'bg-accent/10':''}`}><span><span className="block text-sm font-semibold">{fmtDate(session.created_at)}</span><span className="block text-xs text-secondary mt-0.5 truncate">{session.main_tasks?.[0]||session.claimed_task||'Tracked TELER session'}</span></span><MetricValue value={session.overall_productivity_score} state={session.overall_productivity_score>0?'value':'unscored'} compact /><span className="hidden md:block text-sm text-secondary">{fmtDuration(session.total_minutes)}</span><span className="hidden md:block text-sm text-secondary">{session.app_switches?.length??0} switches</span></DataListLink>})}</DataList>

  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen">
      ...
      <main className="p-4 md:p-6 space-y-5">
        ...
        <section className="bg-surface-card border border-subtle rounded-2xl p-4 flex flex-wrap gap-3 items-center"><div className="flex flex-wrap gap-2">{severityOptions.map(value=><button key={value} type="button" onClick={()=>setSeverity(value)} className={`px-3 py-2 rounded-lg border text-xs ${severity===value?'border-accent bg-accent/10 text-accent':'border-subtle bg-surface-raised text-secondary'}`}>{value==='all'?'All severities':value}</button>)}</div><select aria-label="Alert type" value={type} onChange={event=>setType(event.target.value as Alert['alertType']|'all')} className="bg-surface-raised border border-subtle rounded-lg px-3 py-2 text-xs"><option value="all">All alert types</option>{typeOptions.filter(value=>value!=='all').map(value=><option key={value} value={value}>{ALERT_LABEL[value as Alert['alertType']]}</option>)}</select><div className="ml-auto flex rounded-lg border border-subtle overflow-hidden"><button type="button" onClick={()=>setGroup(false)} className={`px-3 py-2 text-xs ${!group?'bg-accent/10 text-accent':'text-secondary'}`}>All alerts</button><button type="button" onClick={()=>setGroup(true)} className={`px-3 py-2 text-xs ${group?'bg-accent/10 text-accent':'text-secondary'}`}>By employee</button></div></section>

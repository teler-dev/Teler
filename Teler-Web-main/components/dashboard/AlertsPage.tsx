...
const QuickPreview:React.FC<{alert:Alert|null;onClose:()=>void}>=({alert,onClose})=>{
  ...
  return <div className="fixed inset-0 z-[90] flex justify-end" role="dialog" aria-modal="true" aria-label={`Quick preview: ${alert.alertLabel}`}>
    ...
    <aside className="relative w-full max-w-md h-full bg-surface-card border-l border-subtle shadow-2xl flex flex-col">
  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen">
      ...
      <main className="p-4 md:p-6 space-y-5">
        ...
        {loading&&!alerts.length?<div className="space-y-3">{[0,1,2,3].map(i=><div key={i} className="h-20 rounded-2xl bg-surface-card border border-subtle skeleton-shimmer animate-shimmer"/>)}</div>:!alerts.length?<div className="bg-surface-card border border-subtle rounded-2xl px-6 py-14 text-center shadow-card"><div className="w-14 h-14 rounded-2xl bg-success/10 border border-success/20 flex items-center justify-center mx-auto"><ShieldCheck className="w-6 h-6 text-success"/></div><h2 className="text-lg font-semibold mt-5">All clear</h2><p className="text-sm text-secondary mt-2 max-w-md mx-auto">There are no active alerts right now. Filters will appear automatically when there is something to review.</p></div>:!filtered.length?<div className="bg-surface-card border border-subtle rounded-2xl p-10 text-center"><div className="w-12 h-12 rounded-2xl bg-surface-raised border border-subtle flex items-center justify-center mx-auto"><ShieldCheck className="w-5 h-5 text-success"/></div><h2 className="font-semibold mt-4">No alerts in this view</h2><p className="text-sm text-secondary mt-2">No alerts match the current filters.</p></div>:group?<div className="space-y-4">{grouped.map(([employee,items])=><section key={employee} className="bg-surface-card border border-subtle rounded-2xl overflow-hidden"><div className="p-4 border-b border-subtle flex items-center justify-between"><a href={employeePath(employee)} onClick={event=>{if(event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();onEmployeeClick({name:employee,role:items[0]?.employeeRole??'',client:''})}}} className="font-semibold hover:text-accent">{employee}</a><span className="text-xs text-secondary">{items.length} alerts</span></div>{items.map(alert=><AlertRow key={alert.id} alert={alert} onPreview={openPreview}/>)}</section>)}</div>:<section className="bg-surface-card border border-subtle rounded-2xl overflow-hidden">{filtered.map(alert=><AlertRow key={alert.id} alert={alert} onPreview={openPreview}/>)}</section>}
      </main>
    </div>

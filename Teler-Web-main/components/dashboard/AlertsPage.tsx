  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen">
      ...
      <PageContainer>
        ...
        {loading&&!alerts.length?<div className="space-y-3">{[0,1,2,3].map(i=><div key={i} className="h-20 rounded-2xl bg-surface-card border border-subtle skeleton-shimmer animate-shimmer"/>)}</div>:!alerts.length?<EmptyState icon={<ShieldCheck className="w-5 h-5 text-success"/>} title="All clear" description="There are no active alerts right now. Filters will appear automatically when there is something to review." />:!filtered.length?<EmptyState icon={<ShieldCheck className="w-5 h-5 text-success"/>} title="No alerts in this view" description="No alerts match the current filters." />:group?<div className="space-y-4">{grouped.map(([employee,items])=><section key={employee} className="bg-surface-card border border-subtle rounded-2xl overflow-hidden"><div className="p-4 border-b border-subtle flex items-center justify-between"><a href={employeePath(employee)} onClick={event=>{if(event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();onEmployeeClick({name:employee,role:items[0]?.employeeRole??'',client:''})}}} className="font-semibold hover:text-accent">{employee}</a><span className="text-xs text-secondary">{items.length} alerts</span></div>{items.map(alert=><AlertRow key={alert.id} alert={alert} onPreview={openPreview}/>)}</section>)}</div>:<section className="bg-surface-card border border-subtle rounded-2xl overflow-hidden">{filtered.map(alert=><AlertRow key={alert.id} alert={alert} onPreview={openPreview}/>)}</section>}
      </PageContainer>
    </div>

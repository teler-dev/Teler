...
export const GlobalCommandBar: React.FC<Props> = ({ sessions, onNavigate, onEmployee, onOpenAi }) => {
  ...
  const results = useMemo<Result[]>(() => {
    const actions: Result[] = [
      ...
      { id:'workspace', label:'Analytics', description:'Compare workforce performance and supporting evidence', icon:<BarChart3 className="w-4 h-4" />, action:() => onNavigate('workspace') },
  ...
  return <>
    {open && <OverlaySurface label="TELER command bar" onClose={() => setOpen(false)} className="w-full max-w-2xl bg-surface-card border border-subtle rounded-2xl shadow-2xl overflow-hidden">
      <div className="m-3 flex items-center gap-3 px-3 rounded-xl border border-subtle bg-surface-input transition-colors focus-within:border-accent"><Search className="w-5 h-5 text-secondary shrink-0" /><input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} aria-label="Search TELER" placeholder="Search employees, sessions, alerts, actions, settings…" className="flex-1 min-w-0 bg-transparent py-3 outline-none focus-visible:outline-none text-primary placeholder:text-muted text-sm" /><IconButton label="Close command bar" size="sm" variant="ghost" onClick={() => setOpen(false)}><X className="w-4 h-4" /></IconButton></div>
      <div className="border-t border-subtle p-2 max-h-[55vh] overflow-y-auto">{results.length ? results.map(result => <button key={result.id} type="button" onClick={() => run(result)} className="w-full flex items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-surface-raised transition-colors"><span className="w-9 h-9 rounded-xl bg-surface-raised border border-subtle flex items-center justify-center text-secondary shrink-0">{result.icon}</span><span className="min-w-0"><span className="block text-sm font-semibold text-primary truncate">{result.label}</span><span className="block text-xs text-secondary truncate mt-0.5">{result.description}</span></span></button>) : <div className="px-4 py-10 text-center text-sm text-secondary">No matching TELER results.</div>}</div>
    </OverlaySurface>}
  </>;
};
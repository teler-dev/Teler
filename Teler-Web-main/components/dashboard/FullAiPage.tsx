...
export const FullAiPage: React.FC<Props> = ({ onLogout, clientName, onSectionNavigate }) => {
  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen flex flex-col">
      <header className="sticky top-0 z-30 bg-surface-page/90 backdrop-blur-xl border-b border-subtle"><div className="px-4 md:px-6 py-3 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-accent">Workforce Intelligence</p><h1 className="text-2xl md:text-3xl font-bold mt-1">TELER AI Workspace</h1><p className="text-sm text-secondary mt-1 max-w-3xl">Persistent conversations with employee, session, metric and time-range evidence.</p></div><div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto"><button type="button" onClick={newConversation} className="px-4 py-2.5 rounded-xl bg-accent text-white text-sm font-semibold w-full sm:w-auto">New conversation</button><button type="button" onClick={exportConversation} disabled={!messages.length} className="px-4 py-2.5 rounded-xl border border-subtle bg-surface-raised text-sm font-medium disabled:opacity-40 w-full sm:w-auto"><Download className="w-4 h-4 inline mr-1" />Export</button></div></div></header>

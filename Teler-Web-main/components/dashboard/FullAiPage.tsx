      ...
      <div className="flex-1 grid lg:grid-cols-[260px_1fr] min-h-0">
        ...
        <main className="min-w-0 p-4 md:p-6 flex flex-col gap-4 bg-surface-page">
          ...
          <div className="flex-1 min-h-[360px] bg-surface-card border border-subtle rounded-2xl p-4 md:p-6 overflow-y-auto space-y-4 shadow-card">
            ...
            {messages.map((message,index) => <div key={index} className={message.role==='user'?'ml-auto max-w-2xl':'mr-auto max-w-3xl'}><div className={`rounded-xl p-3.5 border text-sm leading-6 whitespace-pre-wrap ${message.role==='user'?'bg-accent-soft border-accent text-primary':message.role==='error'?'bg-danger-soft border-danger text-danger':'bg-surface-raised border-subtle text-primary'}`}>{message.content}</div>{message.sources?.length ? <div className="mt-2 space-y-2"><p className="text-xs text-secondary">Why this conclusion? · {message.confidence}% confidence</p>{message.sources.map(source => <div key={source.id} className="p-3 rounded-xl border border-subtle bg-surface-raised"><div className="flex flex-wrap gap-2 items-center justify-between"><a href={employeePath(source.employee)} onClick={event => { event.preventDefault(); navigate(employeePath(source.employee)); }} className="text-sm font-semibold text-accent">{source.employee}</a><a href={sessionPath(source.employee,source.sessionId)} onClick={event => { event.preventDefault(); navigate(sessionPath(source.employee,source.sessionId)); }} className="text-xs text-secondary hover:text-primary">Session {source.sessionId}</a></div><p className="text-xs text-secondary mt-1">{source.timeRange}</p><div className="flex flex-wrap gap-1.5 mt-2">{source.metrics.map(metric => <span key={metric} className="text-[11px] px-2 py-1 rounded-md bg-surface-raised border border-subtle">{metric}</span>)}</div></div>)}</div> : null}</div>)}
            {loading && <div className="text-sm text-secondary">TELER AI is analyzing the filtered evidence…</div>}<div ref={endRef} />
          </div>
          <div className="bg-surface-card border border-subtle rounded-2xl p-3 md:p-4 shadow-card"><Textarea value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();send(input);} }} aria-label="Ask TELER AI" placeholder="Ask about productivity, risks, sessions or comparisons…" className="min-h-20 border-0 bg-transparent px-1 py-1 focus:border-transparent" /><div className="flex items-center justify-between gap-3 mt-2"><Button variant="ghost" size="sm" onClick={savePrompt} disabled={!input.trim()}>Save prompt</Button><Button size="sm" onClick={() => send(input)} disabled={!input.trim()||loading} aria-label="Send to TELER AI"><Send className="w-4 h-4" />Send</Button></div></div>
        </main>
      </div>
    </div>
  </div>;

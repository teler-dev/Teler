...
import { AiComposer, AiEmptyState, AiSourceCard } from '../ui/AiPrimitives';
  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen flex flex-col">
      <PageHeader
        ...
        actions={<><Button className="justify-center" onClick={newConversation}>New conversation</Button><Button className="justify-center" variant="secondary" onClick={exportConversation} disabled={!messages.length}><Download className="w-4 h-4" />Export</Button></>}
      />
      <div className="flex-1 grid lg:grid-cols-[260px_1fr] min-h-0">
        <aside className="border-b lg:border-b-0 lg:border-r border-subtle p-3 overflow-y-auto bg-surface-card max-h-44 lg:max-h-none"><p className="text-xs font-semibold text-secondary px-2 py-2">Conversation history</p>{conversations.length ? conversations.map(conversation => <div key={conversation.id} className={`group flex items-start gap-1 rounded-xl ${conversation.id===activeId?'bg-accent-soft':'hover:bg-surface-raised'}`}><button type="button" onClick={() => openConversation(conversation)} className="flex-1 text-left px-3 py-2 min-w-0"><span className="block text-sm font-medium truncate">{conversation.title}</span><span className="block text-[11px] text-secondary mt-0.5">{new Date(conversation.createdAt).toLocaleDateString()}</span></button><IconButton label={`Delete ${conversation.title}`} size="sm" variant="ghost" onClick={() => deleteConversation(conversation.id)} className="mt-1 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></IconButton></div>) : <p className="text-xs text-secondary px-2 py-4">No saved conversations yet.</p>}</aside>
        <main className="min-w-0 p-4 md:p-6 flex flex-col gap-4 bg-surface-page">
          <details className="group bg-surface-card border border-subtle rounded-2xl shadow-card">
            <summary className="list-none cursor-pointer px-4 py-3 flex items-center justify-between gap-3">
              <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Context</p><p className="text-sm text-primary mt-0.5">Current workforce scope and filters</p></div>
              <span className="text-xs font-semibold text-accent group-open:hidden">Adjust</span><span className="text-xs font-semibold text-accent hidden group-open:inline">Done</span>
            </summary>
            <div className="p-3 pt-0"><WorkspaceToolbar sessions={sessions} compact /></div>
          </details>
          <div className="flex-1 min-h-[360px] bg-surface-card border border-subtle rounded-2xl p-4 md:p-6 overflow-y-auto space-y-4 shadow-card">
            {!messages.length && <AiEmptyState>{savedPrompts.length > 0 && <div className="flex flex-wrap justify-center gap-2 mt-5">{savedPrompts.map(prompt => <button key={prompt} type="button" onClick={() => send(prompt)} className="text-xs px-3 py-2 rounded-xl border border-subtle bg-surface-raised hover:bg-surface-hover">{prompt}</button>)}</div>}</AiEmptyState>}
            {messages.map((message,index) => <div key={index} className={message.role==='user'?'ml-auto max-w-2xl':'mr-auto max-w-3xl'}><div className={`rounded-xl p-3.5 border text-sm leading-6 whitespace-pre-wrap ${message.role==='user'?'bg-accent-soft border-accent text-primary':message.role==='error'?'bg-danger-soft border-danger text-danger':'bg-surface-raised border-subtle text-primary'}`}>{message.content}</div>{message.sources?.length ? <div className="mt-2 space-y-2"><p className="text-xs text-secondary">Why this conclusion? · {message.confidence}% confidence</p>{message.sources.map(source => <AiSourceCard key={source.id} source={source} onEmployee={() => navigate(employeePath(source.employee))} onSession={() => navigate(sessionPath(source.employee,source.sessionId))} />)}</div> : null}</div>)}
            {loading && <div className="text-sm text-secondary">TELER AI is analyzing the filtered evidence…</div>}<div ref={endRef} />
          </div>
          <AiComposer value={input} onChange={setInput} onSend={() => send(input)} onKeyDown={event => { if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();send(input);} }} disabled={loading} placeholder="Ask about productivity, risks, sessions or comparisons…" leadingAction={<Button variant="ghost" size="sm" onClick={savePrompt} disabled={!input.trim()}>Save prompt</Button>} />
        </main>
      </div>
    </div>
  </div>;

...
import { Select, TextInput } from '../ui/FormControls';
  ...
  return <div className={`bg-surface-card border border-subtle rounded-2xl ${compact ? 'p-3' : 'p-4 md:p-5'} shadow-card`}>
    <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2.5">
      <div className="flex items-center gap-2 text-primary mr-1">
        <span className="w-8 h-8 rounded-xl border border-subtle bg-surface-raised flex items-center justify-center"><Filter className="w-4 h-4 text-accent" /></span>
        <span className="text-xs font-semibold uppercase tracking-[0.12em]">Filters</span>
      </div>

      {selectWrap('Date range',
        <Select className="w-full sm:w-auto text-xs" value={filters.days} onChange={event => update('days',Number(event.target.value))}>
      ...
      {selectWrap('Team',
        <Select className="w-full sm:w-auto text-xs" value={filters.team} onChange={event => update('team',event.target.value)}>
          <option value="all">All teams</option>{teams.map(value=><option key={value}>{value}</option>)}
        </Select>
      )}

      <details className="group relative w-full sm:w-auto">
        <summary className="list-none cursor-pointer min-h-10 px-3 rounded-xl border border-subtle bg-surface-raised text-secondary hover:text-primary hover:border-strong text-xs font-semibold flex items-center justify-center gap-2">
          <SlidersHorizontal className="w-3.5 h-3.5" />
          More filters
          <ChevronDown className="w-3 h-3 transition-transform group-open:rotate-180" />
        </summary>
        <div className="mt-2 sm:absolute sm:left-0 sm:top-full sm:z-40 sm:min-w-[520px] grid sm:grid-cols-2 gap-2 rounded-2xl border border-subtle bg-surface-card p-3 shadow-card">
          {selectWrap('Role',
            <Select className="w-full text-xs" value={filters.role} onChange={event => update('role',event.target.value)}>
              <option value="all">All roles</option>{roles.map(value=><option key={value}>{value}</option>)}
            </Select>
          )}
          {selectWrap('Risk level',
            <Select className="w-full text-xs" value={filters.risk} onChange={event => update('risk',event.target.value as WorkspaceFilters['risk'])}>
          ...
          {selectWrap('Timezone',
            <Select className="w-full text-xs" value={filters.timezone} onChange={event => update('timezone',event.target.value)}>
              {timezones.map(value=><option key={value}>{value}</option>)}
            </Select>
          )}
          {selectWrap('Comparison period',
            <Select className="w-full text-xs" value={filters.compare} onChange={event => update('compare',event.target.value as WorkspaceFilters['compare'])}>
      ...
      <Button variant="outline" size="sm" onClick={() => setSaving(value=>!value)} className="w-full sm:w-auto min-h-10">
    ...
    {saving && <div className="flex flex-col sm:flex-row gap-2 mt-3 max-w-md">
      <TextInput aria-label="Saved view name" value={viewName} onChange={event=>setViewName(event.target.value)} placeholder="e.g. Weekly engineering risk" />
      <Button size="sm" onClick={saveCurrent} className="min-h-10">Save</Button>
    </div>}

    {views.length>0 && <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-subtle"><span className="text-xs text-muted py-1">Saved views</span>{views.slice(0,6).map(view=><div key={view.id} className="flex items-center rounded-xl bg-surface-raised border border-subtle overflow-hidden"><button type="button" onClick={()=>setFilters(view.filters)} className="px-2.5 py-1.5 text-xs text-primary hover:bg-surface-hover">{view.name}</button><IconButton label={`Delete ${view.name}`} size="sm" variant="ghost" onClick={()=>{deleteSavedView(view.id);setViews(getSavedViews())}}><Trash2 className="w-3 h-3" /></IconButton></div>)}</div>}
  </div>;
};
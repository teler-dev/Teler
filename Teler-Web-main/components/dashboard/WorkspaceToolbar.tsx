...
export const WorkspaceToolbar: React.FC<Props> = ({ sessions, compact = false }) => {
  ...
  return <div className={`bg-surface-card border border-subtle rounded-2xl ${compact ? 'p-3' : 'p-4 md:p-5'} shadow-card`}>
    <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2.5">
      <div className="flex items-center gap-2 text-primary mr-1 mb-1 sm:mb-0"><span className="w-8 h-8 rounded-lg border border-subtle bg-surface-raised flex items-center justify-center"><Filter className="w-4 h-4 text-accent" /></span><span className="text-xs font-semibold uppercase tracking-[0.12em]">Filters</span></div>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Date range</span><select className={selectClass} value={filters.days} onChange={event => update('days',Number(event.target.value))}><option value={7}>Last 7 days</option><option value={14}>Last 14 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option><option value={9999}>All time</option></select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Team</span><select className={selectClass} value={filters.team} onChange={event => update('team',event.target.value)}><option value="all">All teams</option>{teams.map(value=><option key={value}>{value}</option>)}</select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Role</span><select className={selectClass} value={filters.role} onChange={event => update('role',event.target.value)}><option value="all">All roles</option>{roles.map(value=><option key={value}>{value}</option>)}</select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Risk level</span><select className={selectClass} value={filters.risk} onChange={event => update('risk',event.target.value as WorkspaceFilters['risk'])}><option value="all">All risk</option><option value="healthy">Healthy</option><option value="attention">Needs attention</option><option value="high">High risk</option></select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Timezone</span><select className={selectClass} value={filters.timezone} onChange={event => update('timezone',event.target.value)}>{timezones.map(value=><option key={value}>{value}</option>)}</select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
      <label className="relative w-full sm:w-auto"><span className="sr-only">Comparison period</span><select className={selectClass} value={filters.compare} onChange={event => update('compare',event.target.value as WorkspaceFilters['compare'])}><option value="previous_period">Previous period</option><option value="previous_week">Previous week</option><option value="previous_month">Previous month</option></select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-muted pointer-events-none" /></label>
    ...
    <p className="text-[11px] text-muted mt-3 leading-5">These filters are reflected in the URL, so the current analysis can be bookmarked or shared.</p>

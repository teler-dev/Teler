...
export const EmployeesPage:React.FC<Props>=({onLogout,onEmployeeClick,onSectionNavigate,clientName='Your Company'})=>{
  ...
  return <div className="min-h-screen bg-surface-page text-primary flex">
    ...
    <div className="flex-1 ml-56 min-w-0 min-h-screen">
      <header className="sticky top-0 z-30 bg-surface-page/90 backdrop-blur-xl border-b border-subtle"><div className="px-4 md:px-6 py-3 flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-accent">Workforce Intelligence</p><h1 className="text-2xl md:text-3xl font-bold mt-1">Employees</h1><p className="text-sm text-secondary mt-1 max-w-2xl">{rows.length} team members · {counts.working} working · {counts.idle} idle · {counts.offline} offline</p></div><button type="button" onClick={()=>refetch(true)} aria-label="Refresh employees" title="Refresh employees" className="w-10 h-10 rounded-lg border border-subtle bg-surface-raised text-secondary flex items-center justify-center"><RefreshCw className={`w-4 h-4 ${loading?'animate-spin':''}`}/></button></div></header>
